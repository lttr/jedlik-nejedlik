...
| #   | Finding                                                                                                                                                     | Why not                                                                                                                                                                                                  |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3   | The online course's buy link navigates in the same tab, so its `InitiateCheckout` races the unload and under-reports against the two `target="_blank"` ones | **Not a real risk (checked 2026-09-13)** &mdash; `fbevents.js` sends via `fetch` with `keepalive: true` (fallbacks `sendBeacon`, `formPOST`; no image beacon), so a dispatched event outlives the unload |
...
| 11  | No `PageView` on client-side route changes, so Meta sees one PageView per visit and cannot build URL-based audiences                                        | **Fixed 2026-09-13** &mdash; a `router.afterEach` in the plugin sends one per SPA navigation, behind the same consent gate                                                                               |
