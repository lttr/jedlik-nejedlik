...
- `rolldown` is an explicit `web/` devDependency because `nuxt` peers `rolldown: ~1.2.1` and does NOT mark it optional (`@nuxt/vite-builder` peers `^1.0.0` too). It is required with or without the vite-plus alias, so it does not come out when that override does. The pin is exact and tracks whatever the catalog's `vite-plus` release bundles (0.3.0 → 1.2.5) — never widen it to a range; rolldown ships a native binary and a skew against vite-plus's own copy is what `vitest-upstream` exists to route around.
