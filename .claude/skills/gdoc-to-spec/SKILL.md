...
- The exact next command:
  ...
  ```
  /aiwork:implement .aiwork/{date}_{slug}/spec.md
  ```

Don't invoke `/aiwork:implement` yourself — the user runs it when ready. For a spec big enough to need a ticket graph, the next command is `/aiwork:to-tickets` and then `/aiwork:implement-spec`.
